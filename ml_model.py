...
data['day'] = data['date_reported'].apply(lambda x: x.timetuple().tm_yday)
data['week'] = data['date_reported'].dt.week
data['month'] = data['date_reported'].dt.month
data['season'] = (data['month'] % 12 + 3) // 3

def get_forecasts_ml(country):
    global data
    india_data = data.loc[data['country']==country,['date_reported', 'day','week', 'month', 'season', 'deaths'] ].copy()
